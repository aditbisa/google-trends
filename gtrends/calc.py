...
def calculate(start_date: datetime.date, stop_date: datetime.date) -> Tuple[int, bool]:
    """
    ...
    time_delta: timedelta = stop_date - start_date
...
def sanitize_command() -> Tuple:
    ...
    if len(sys.argv) <= 1:
        print(f"Usage:\n  python {__file__} <start-date> <stop-date, default: today>")
        print(f"Example:\n  python {__file__} 2015-01-01 2018-01-01")
        exit(1)

    start_date_str = sys.argv[1]
    stop_date_str = TODAY_STRING if len(sys.argv) < 3 else sys.argv[2]

    try:
        start_date = datetime.strptime(start_date_str, DATE_FORMAT).date()
        if stop_date_str == TODAY_STRING:
